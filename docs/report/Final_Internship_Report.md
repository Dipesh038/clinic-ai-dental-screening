...
The internship was structured as a four-phase, task-tracked development cycle (authentication and CRUD; image upload and AI integration; dentist review, PDF reporting, and dashboard; and final hardening, deployment, and documentation), with disciplined use of a living project-status log to track completed work, outstanding risks, and discovered defects. Nine real defects were identified and resolved during development, most notably a data-model mismatch that would have caused the PDF report endpoint to fail on every real invocation, and multiple WCAG AA colour-contrast violations found via automated accessibility auditing. The final YOLOv8n model achieved a validation mAP50 of 0.837 (83.7%) at 2.6 ms inference latency per image.
...
- **Grad-CAM explainability (stretch goal):** for each detected region, a Grad-CAM heatmap can be generated (via a pretrained ResNet18 classifier applied to the cropped detection) and toggled on in the review UI, visualizing which pixels most influenced the classification.
...
- Nine distinct defects identified and resolved during development (§4.1.7 and the project status log), each documented with root cause and fix rather than only a patch.
